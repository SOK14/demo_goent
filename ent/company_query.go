...
func (cq *CompanyQuery) QueryEmployee() *UserQuery {
...
func (cq *CompanyQuery) WithEmployee(opts ...func(*UserQuery)) *CompanyQuery {
	query := &UserQuery{config: cq.config}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	if query := cq.withEmployee; query != nil {
		if err := cq.loadEmployee(ctx, query, nodes,
			func(n *Company) { n.Edges.Employee = []*User{} },
			func(n *Company, e *User) { n.Edges.Employee = append(n.Edges.Employee, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CompanyQuery) loadEmployee(ctx context.Context, query *UserQuery, nodes []*Company, init func(*Company), assign func(*Company, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(company.EmployeeColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "company_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
