...
// AddEmployee adds the "employee" edges to the User entity.
func (cu *CompanyUpdate) AddEmployee(u ...*User) *CompanyUpdate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
// ClearEmployee clears all "employee" edges to the User entity.
func (cu *CompanyUpdate) ClearEmployee() *CompanyUpdate {
	cu.mutation.ClearEmployee()
	return cu
}

// RemoveEmployeeIDs removes the "employee" edge to User entities by IDs.
func (cu *CompanyUpdate) RemoveEmployeeIDs(ids ...int) *CompanyUpdate {
	cu.mutation.RemoveEmployeeIDs(ids...)
	return cu
}

// RemoveEmployee removes "employee" edges to User entities.
func (cu *CompanyUpdate) RemoveEmployee(u ...*User) *CompanyUpdate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (cu *CompanyUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := cu.mutation.RemovedEmployeeIDs(); len(nodes) > 0 && !cu.mutation.EmployeeCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   company.EmployeeTable,
...
// AddEmployee adds the "employee" edges to the User entity.
func (cuo *CompanyUpdateOne) AddEmployee(u ...*User) *CompanyUpdateOne {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
// ClearEmployee clears all "employee" edges to the User entity.
func (cuo *CompanyUpdateOne) ClearEmployee() *CompanyUpdateOne {
...
// RemoveEmployee removes "employee" edges to User entities.
func (cuo *CompanyUpdateOne) RemoveEmployee(u ...*User) *CompanyUpdateOne {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (cuo *CompanyUpdateOne) sqlSave(ctx context.Context) (_node *Company, err error) {
	...
	if nodes := cuo.mutation.RemovedEmployeeIDs(); len(nodes) > 0 && !cuo.mutation.EmployeeCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   company.EmployeeTable,
